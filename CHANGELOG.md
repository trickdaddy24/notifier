...
- Web UI Phase 2 release: Full web interface with secure authentication (bcrypt + signed cookies), username + password login, functional Add Reminder modal, real-time reminder listing and deletion. Major database refactor: introduced shared notifier/db.py module used by both CLI and web. Improved Docker support with proper lifespan-based DB initialization. Added ROADMAP for future multi-user + Google OAuth work.
